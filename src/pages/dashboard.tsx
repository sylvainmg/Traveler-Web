...
import clients, { getClientInfo } from "../api/client.ts";
...
function Dashboard({ token }: { token: string }) {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            if (userData) {
                ...
                // Stats
                const total =
                    userData.total || userData.total || userReservations.length;
    ...
    const handleGiveReview = (reservation: any) => {
        ...
        const normalizedReservation = {
            ...
            duree_sejour:
                reservation.duree_sejour || reservation.DUREE_SEJOUR || 0,
            destination:
                reservation.destination || reservation.DESTINATION || "",
            pays_depart:
                reservation.pays_depart || reservation.PAYS_DEPART || "",
            compagnie_aerienne_aller:
                reservation.compagnie_aerienne_aller ||
                reservation.COMPAGNIE_AERIENNE_ALLER ||
                "",
            compagnie_aerienne_retour:
                reservation.compagnie_aerienne_retour ||
                reservation.COMPAGNIE_AERIENNE_RETOUR ||
                "",
            prix_aller: reservation.prix_aller || reservation.PRIX_ALLER || "0",
            prix_retour:
                reservation.prix_retour || reservation.PRIX_RETOUR || "0",
            prix_chambre:
                reservation.prix_chambre || reservation.PRIX_CHAMBRE || "0",
            prix_total:
        ...
        localStorage.setItem(
            "tempReservationData",
            JSON.stringify(normalizedReservation)
        );
        localStorage.setItem(
            "tempReservationId",
            String(normalizedReservation.num_reservation)
        );
                    ...
                    {reservations.length > 0 ? (
                        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-1">
                            {reservations.map((r, i) => {
                                const debut =
                                    r.debut_sejour || r.DEBUT_SEJOUR || "";
                                const duree =
                                    r.duree_sejour || r.DUREE_SEJOUR || 0;
                                const destination =
                                    r.destination ||
                                    r.DESTINATION ||
                                    "Destination";
                                ...
                                const numReservation =
                                    ...
                                    i + 1;
                                const paysDepart =
                                    r.pays_depart || r.PAYS_DEPART || "N/A";
                                const compAller =
                                    r.compagnie_aerienne_aller ||
                                    r.COMPAGNIE_AERIENNE_ALLER ||
                                    "N/A";
                                const compRetour =
                                    r.compagnie_aerienne_retour ||
                                    r.COMPAGNIE_AERIENNE_RETOUR ||
                                    "N/A";
                                const prixAller =
                                    r.prix_aller || r.PRIX_ALLER || "0";
                                const prixRetour =
                                    r.prix_retour || r.PRIX_RETOUR || "0";
                                const prixChambre =
                                    r.prix_chambre || r.PRIX_CHAMBRE || "0";
                                const prixTotal =
                                    r.prix_total ||
                                    r.total ||
                                    r.PRIX_TOTAL ||
                                    "0";
                                const nbEtoile =
                                    r.hotel?.nb_etoile || r.nb_etoile || 0;
                        ...
                        <div className="bg-white p-12 rounded-2xl text-center shadow-md">
                            ...
                            <p className="text-gray-600 mb-6">
                                Vous n'avez pas encore de réservation. Commencez
                                votre aventure !
                            </p>
                            <button
